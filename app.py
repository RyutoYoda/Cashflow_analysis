...
def fetch_stock_data_yfinance(ticker):
    try:
        ...
        hist = stock.history(period="1y")  # 過去1年分のデータ
...
stock_ticker = st.text_input("Yahoo Financeのティッカーシンボルを入力してください", "4755.T")  # 例: 楽天のティッカーシンボル
...
if st.button("分析開始"):
    ...
    # 株価データの取得と表示
    stock_data = fetch_stock_data_yfinance(stock_ticker)
